...
def MakeMagnifiedGainComponentEmbeddings(input_folder, output_folder):
  ...
  for f in raw_embeddings:
    ...
    # For each component, double the magnitude and save the modified array.
    for i in range(16):
      encoding_cp = np.copy(encoding)
      encoding_cp[:,i] *= 2.0

      # Save a modified array for this component.
      outfile = os.path.join(output_folder, 'gain2x_%d_%s' % (i, tail))
      np.save(outfile, encoding_cp)

if __name__ == '__main__':
  base_dir = '/home/milo/mit/21M.080-music-tech/wavenet-embeddings/generated/02_gain_analysis'
  ...
  MakeMagnifiedGainComponentEmbeddings(input_folder, output_folder)
